...
def album_to_html_table(album):
    ...
    for track in album['tracks']:
        tracks_table += '<tr><td>{}</td><td>{}</td></tr>'.format(
...
def display_album(album):
    ...
    for tr in result['items']:
        tracks.append(
            {"name": tr['name'], "duration": format_time(tr['duration_ms'])})
...
def display_playlist(playlist):
    ...
    result = spotify_request('GET', 'playlists/' +
                             playlist['id'] + '/tracks', {"limit": 20})
...
def display_artist(artist):
    ...
    result = spotify_request(
        'GET', 'artists/' + artist['id'] + '/top-tracks', {'country': user_info['country']})

    top_tracks = []
    for tr in result['tracks']:
        top_tracks.append({"name": tr['name'], "duration": format_time(
            tr['duration_ms']), "artist": artist['name']})
...
def get_current_track():
    data = {}
    currently_playing = spotify_request(
        'GET', 'me/player/currently-playing', {})['item']
