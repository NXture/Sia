...
export default () => new Promise(async (resolve, reject) => {
  try {
    ...
    if (!fs.existsSync(conquiLanguageModelPath)) {
      report.can_offline_stt.v = false
      log.warning(`Cannot find ${conquiLanguageModelPath}. You can setup the offline STT by running: "npm run setup:offline-stt"`)
    } else {
      log.success(`Found Coqui language model at ${conquiLanguageModelPath}`)
    }
